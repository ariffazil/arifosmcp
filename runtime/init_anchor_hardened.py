...
class HardenedInitAnchor:
    ...
    def _load_registry(self) -> dict[str, Any]:
        ...
        registry_path = next((p for p in base_paths if os.path.exists(p) and os.path.isdir(p)), None)
        ...
        comprehensive = {"catalog": {}, "souls": {}, "models": {}, "profiles": {}, "layer": "4-layer"}
        try:
            # 1. Load Catalog
    ...
    def _bind_identity(
        ...
        matched_profile = profiles.get(deployment_id) if deployment_id else next(
            (p for p in profiles.values() if p.get("model_id") == variant), None
        )
        if matched_profile:
            binding["runtime"] = {
        ...
        sid = (matched_model_spec.get("soul_archetype") if matched_model_spec else None) or \
              f"{(cp or provider)}_{(cf or family)}".lower()
        soul = souls.get(sid, souls.get(f"{provider}_{family}", souls.get(family, souls.get(provider, {}))))
        ...
        if soul:
            ...
            if matched_model_spec and claim_matches:
                binding["verified_identity"] = {
                    ...
                    "trust_tier": matched_model_spec.get("identity_integrity", {}).get("trust_tier"),
                }
    ...
    async def init(
        ...
        **kwargs: Any
        ...
        if _aid and _dn and _aid != _dn: declared_name_norm = _aid
        eff_intent = str(intent or query or raw_input or f"Init {declared_name_norm}")
        if not session_id: session_id = f"sess-{secrets.token_hex(8)}"
        ...
        challenge = SignedChallenge(
            f"chal-{secrets.token_hex(8)}", declared_name_norm, eff_intent[:200],
            req_scope, risk, sclass, datetime.now(timezone.utc).isoformat(), secrets.token_hex(16)
        )
        ...
        state = SessionState(
            session_id, datetime.now(timezone.utc).isoformat(), datetime.now(timezone.utc).isoformat(),
            declared_name_norm, sclass, req_scope, risk, challenge.compute_hash()
        )
        self._sessions[session_id] = state
        bind_session_identity(
            session_id, declared_name_norm, sclass.value, effective_auth_ctx,
            req_scope, bool(human_approval), "anchored"
        )
        ...
        return ToolEnvelope(
            status=ToolStatus.OK, tool="init_anchor", session_id=session_id, risk_tier=risk,
            confidence=0.95, trace=trace,
            entropy=EntropyBudget(ambiguity_score=0.0, delta_s=0.0, confidence=1.0),
            authority=authority_obj, caller_state="anchored", auth_context=effective_auth_ctx,
            allowed_next_tools=next_allowed,
            payload={
                "identity": {
                    "declared_actor_id": declared_name_norm, "auth_state": auth_state,
                    ...
                    "self_claim_boundary": binding["boundary"], "note": binding["note"]
                },
                "bound_session": {
                    "soul": binding["soul"], "runtime": binding["runtime"],
                    "boundary": binding["boundary"], "bound_role": binding["bound_role"]
                },
    ...
    async def state(self, session_id: str, trace: TraceContext | None = None) -> ToolEnvelope:
        ...
        bootstrap_data = {
            "bootstrap_sequence": ["1. check_vital", "2. audit_rules", "3. init_anchor", "4. arifOS_kernel"],
            "system_motto": motto
        }
        if session_id not in self._sessions:
            next_allowed = self._get_next_tools(SessionClass.OBSERVE)
            return ToolEnvelope(
                status=ToolStatus.OK, tool="init_anchor", session_id=session_id or "global",
                risk_tier=RiskTier.LOW, confidence=1.0, trace=trace,
                entropy=EntropyBudget(0.0, 0.0, 1.0), caller_state="anonymous",
                allowed_next_tools=next_allowed,
                payload={"reason": "Session not found", "caller_state": "anonymous", "bootstrap": True, **bootstrap_data}
            )
        ...
        return ToolEnvelope(
            status=ToolStatus.OK, tool="init_anchor", session_id=session_id, risk_tier=state.risk_tier,
            confidence=0.90, trace=trace, entropy=EntropyBudget(0.0, 0.0, 0.90),
            authority=authority_obj, caller_state="anchored", auth_context=identity.get("auth_context"),
            allowed_next_tools=next_allowed,
            payload={
    ...
    async def revoke(self, session_id: str, reason: str = "User request") -> ToolEnvelope:
        ...
        return ToolEnvelope(
            status=ToolStatus.OK, tool="init_anchor", session_id=session_id,
